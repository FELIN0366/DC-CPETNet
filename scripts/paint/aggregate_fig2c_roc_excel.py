...
用途：扫描 results/fig2c_roc_raw/*.json，聚合五折 ROC 数据，生成 Excel
...
def aggregate_fold_roc_data(input_dir: str, output_excel: str):
    """
    ...
    for model_type in auc_by_model_task_fold:
        for task_key in auc_by_model_task_fold[model_type]:
            ...
            # 获取模型名称和任务名称
            model_name = sample_scores_df[sample_scores_df["model_type"] == model_type]["model_name"].iloc[0] if len(sample_scores_df) > 0 else model_type
...
def main():
    ...
    # 切换到项目目录 (scripts/paint/ 是两层嵌套)
    project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    os.chdir(project_root)
    print(project_root)

    aggregate_fold_roc_data(args.input_dir, args.output_excel)
