"""
AdaTT Clean Training Plan - Training stages for AdaTT architecture

This module defines the training stages for AdaTT clean architecture.
...
AdaTT specific:
- No Alpha/Beta branch freeze (AdaTT has no branches)
- Each task has its own stream, frozen by task-specific module names
...
class StageConfig:
    ...
    Attributes:
        ...
        t6_context_inject: Whether to inject t6 context (always False for AdaTT)
        t6_context_detach: Whether to detach t6 context gradients (always False for AdaTT)
        use_uncertainty_weighting: Uncertainty weighting toggle
    """
    name: str
    epochs: int
    ...
    t6_context_inject: bool = False  # Always False for AdaTT
    t6_context_detach: bool = False  # Always False for AdaTT
    use_uncertainty_weighting: bool = False  # Uncertainty weighting toggle


# =============================================================================
# AdaTT Clean Training Plan
# =============================================================================

class AdaTTCleanTrainingPlan:
    """
    AdaTT Clean Training Plan.

    Defines the training stages for AdaTT architecture:
    - No Stage0 (no t6 KD)
    - No t6 context injection
    - Standard multi-task progression
    - Task-specific stream freeze/unfreeze for staged training

    Stage Flow:
        Stage1: t1 only (anchor AdaTT streams for t1)
    ...
    4. Task-specific module freeze (e.g., task_heads_t2, classifiers_t3)
    5. AdaTT fusion layers always trainable (shared knowledge building)
    ...
    plan_name: str = "adatt_clean_plan"
    architecture_variant: str = "adatt_clean_v4"

    # AdaTT specific flags
    uses_t6_context: bool = False
    uses_feature_distillation: bool = False
    uses_stage0_kd: bool = False

    def __init__(self):
        """Initialize AdaTT training plan with explicit stage configs."""
        self.stages = self._build_stages()
        print(f"\n[AdaTT Clean Plan] Training plan initialized:")
    ...
    def _build_stages(self) -> Dict[str, StageConfig]:
        """Build training stages for AdaTT."""
        stages = {}

        # Stage1: t1 anchor
        # - Train t1 with AdaTT task-to-task fusion
        # - Freeze all other task-specific components (t2-t5 heads, encoders, classifiers)
        # - AdaTT fusion layers remain trainable (shared knowledge building)
        stages["stage1"] = StageConfig(
            ...
            loss_weights={"t1": 1.0},
            freeze_modules=[
                # Freeze t2-t5 task-specific components
                ...
                # Note: task_adapters for t2-t5 are frozen via task_heads_* pattern
            ],
        ...
        # Stage2: t2-t5 warmup
        # - Train t2-t5 tasks with AdaTT
        # - Freeze t1 components
        # - AdaTT fusion layers continue training (cross-task knowledge building)
        stages["stage2"] = StageConfig(
            ...
            freeze_modules=[
                "task_heads_t1", "static_encoders_t1", "classifiers_t1"
            ],
        ...
        # - No freezing (all AdaTT components trainable)
...
def get_adatt_clean_plan() -> AdaTTCleanTrainingPlan:
    """
    Factory function to create AdaTT Clean Training Plan.

    Returns:
        AdaTTCleanTrainingPlan instance
    """
    return AdaTTCleanTrainingPlan()


def validate_adatt_plan(plan: AdaTTCleanTrainingPlan) -> bool:
    """
    Validate AdaTT training plan.
    ...
    Args:
        plan: AdaTTCleanTrainingPlan instance
    ...
    if "stage0" in plan.stages:
        print(f"[Validation ERROR] stage0 exists (AdaTT should not have Stage0)")
        return False

    # Check architecture binding
    if plan.architecture_variant != "adatt_clean_v4":
        print(f"[Validation ERROR] architecture_variant={plan.architecture_variant} (should be adatt_clean_v4)")
        return False

    print(f"[Validation OK] AdaTT training plan valid")
    return True
...
if __name__ == "__main__":
    print("=" * 60)
    print("AdaTT Clean Training Plan Test")
    print("=" * 60)

    # Create plan
    plan = get_adatt_clean_plan()
    print(f"\n{plan}")

    # Validate plan
    is_valid = validate_adatt_plan(plan)
